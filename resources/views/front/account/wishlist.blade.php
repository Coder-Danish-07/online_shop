                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="d-sm-flex justify-content-between mt-lg-4 mb-4 pb-3 pb-sm-2 border-bottom">
                            <div class="d-block d-sm-flex align-items-start text-center text-sm-start">
                                ...
                                @php
                                $productImage = ProductImage($wishlist->product->id);
                                @endphp
                                @if(!empty($productImage->image))
                                <img src="{{ asset('uploads/product/small/'.$productImage->image)}}" class="card-img-top"  >
                                @else
                                <img src="{{asset('admin-assets/img/default-image.png')}}"  class="card-img-top">
                                @endif
                                </a>
                                <div class="pt-2">
                            ...
                            <div class="pt-2 ps-sm-3 mx-auto mx-sm-0 text-center">
                                <button onclick="removeWishlist({{ $wishlist->product->id }})" class="btn btn-outline-danger btn-sm" type="button"><i class="fas fa-trash-alt me-2"></i>Remove</button>
                            </div>
                        </div>
                        @endforeach
                        @else
